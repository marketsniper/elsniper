...
describe('Trajets partagés (rides)', () => {
  ...
  it('cloison tarifaire : touriste 16 USD, résident vérifié 14,40 USD, local TZS', async () => {
    const { token } = await createVerifiedDriver();
    const ride = (await postRide(token)).body;
    ...
    assert.equal(r.price_per_seat_usd, 14.4);
...
describe('Trajets partagés — réservation de places dans l\'app', () => {
  it('un touriste réserve 2 places → décompte automatique, trace et notification', async () => {
    ...
    assert.equal(mine.body[0].bookings[0].price_per_seat, 16); // Nungwi : privé 47 → place 15
    assert.equal(mine.body[0].bookings[0].currency, 'USD');
    assert.equal(mine.body[0].bookings[0].commission_per_seat, 4); // 22 % de 15
    assert.equal(mine.body[0].bookings[0].net_per_seat, 12); // le chauffeur garde 78 %
  });
