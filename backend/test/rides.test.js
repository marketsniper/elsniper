...
describe('Trajets partagés — réservation de places dans l\'app', () => {
  it('un touriste réserve 2 places → décompte automatique, trace et notification', async () => {
    ...
    assert.equal(mine.body[0].bookings[0].price_per_seat, 15); // Nungwi : privé 47 → place 15
    assert.equal(mine.body[0].bookings[0].currency, 'USD');
    assert.equal(mine.body[0].bookings[0].commission_per_seat, 3.3); // 22 % de 15
    assert.equal(mine.body[0].bookings[0].net_per_seat, 11.7); // le chauffeur garde 78 %
  });
