...
describe('Devises taxi partagé (parcours local complet)', () => {
  ...
  it('téléphone équipe : l\'identité CLIENT prime sur la clé admin (touriste = USD)', async () => {
    ...
    // La place vaut 15 USD ; réglée par carte (le choix par défaut d'un
    ...
    assert.equal(Number(resa.body.payment.amount), 15.6);
    assert.equal(Number(resa.body.payment.surcharge), 0.6);
    ...
    assert.equal(Number(mesPlaces.body[0].amount), 15, 'le PRIX de la place');
    // …et, à côté, ce qu'il y a réellement à régler avec le moyen choisi.
    assert.equal(Number(mesPlaces.body[0].reglement_montant), 15.6);
  ...
  it('trajet spécial local : Nungwi ↔ Paje à 21 000 TZS la place (deux sens)', async () => {
    const { token: tokenChauffeur } = await createVerifiedDriver();
    const depart = new Date(Date.now() + 6 * 3600 * 1000).toISOString();
  ...
  it('tarif local UNIQUEMENT sur les grands axes (privé ≥ 40 USD) — ailleurs, prix touriste en TZS', async () => {
    ...
    // autorisé ET place locale à 16 000.
...
describe('Paiement des places de taxi partagé', () => {
  it('réservation → paiement pending au tableau équipe ; confirmation → place payée', async () => {
    ...
    // Un local réserve 2 places → paiement en attente de 2 × 16 000 TZS.
    const resa = await request(app)
      .post(`/api/rides/${posted.body.id}/book`)
      .set(authHeaders(tokenLocal))
      .send({ seats: 2 });
