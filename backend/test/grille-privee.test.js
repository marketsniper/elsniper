...
describe('Grille privée au kilomètre', () => {
  ...
  it('un village d’écart : 16 USD, sur les trois paires concernées', () => {
    ...
    for (const [a, b] of ecarts) {
      ...
      assert.equal(saut.commission, 2.4, `${a} → ${b} : commission 15 %`);
  ...
  it('sauts de village : 12 USD à 15 % — le chauffeur garde 10,20', () => {
    ...
    // Un transfert plus long : même taux de 15 % (Matemwe → Paje 40 → 6).
  ...
  it('Kizimkazi ↔ Jambiani : 17 USD, la route passe par l’intérieur', () => {
    ...
    assert.equal(t.commission, 2.55, 'commission privée 15 %');
  ...
  it('Michamvi depuis le nord : 65 USD, la route contourne la baie', () => {
    ...
    assert.equal(traversee.commission, 9.75, 'commission privée 15 %');
    assert.equal(traversee.price - traversee.commission, 55.25, 'gain chauffeur');
