...
describe('Chauffeur — compteur de gains', () => {
  it('courses terminées et colis livrés comptés avec gains nets ; autre chauffeur → 403', async () => {
    ...
    // Kendwa suit Nungwi : transfert 42 USD (baisse du 05/09/2026, réunion
    // chauffeurs), commission 12 % → 5,03, net versé 36,97 USD.
    assert.equal(stats.body.today.gains.USD, 36.97);
    ...
    // net zanziGo = commission (transfert Kendwa 42 USD, 12 % → 5,03).
    ...
    assert.equal(equipe.body.revenue.today.ca.USD, 42);
    assert.equal(equipe.body.revenue.today.gains.USD, 5.03);
